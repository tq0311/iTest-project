		...
		<div class="result-card">
			...
			<div class="content">
				<div class="table-list">
					...
					<el-dialog title="添加任务--短信管理" :visible.sync="dialogFormVisible" :modal="false" :close-on-click-modal="false" :lock-scroll="true">
						<el-form :model="form">
					...
					<div class="view">
						<el-dialog title="修改任务--短信管理" :visible.sync="dialogFormVisible1" :modal="false" :close-on-click-modal="false" :lock-scroll="true">
							<el-form :model="form1">
							</el-form>
							<div slot="footer" class="dialog-footer">
	...
	export default {
		...
		methods: {
			...
			edit () {
				...
				var that = this
				this.axios.get(`${this.url}/system/smsmanage/update`)
			...
			addData () {
				...
				var that = this
				this.axios.get(`${this.url}/system/smsmanage`)
				.then(function (response) {
			...
			deleteData () {
				if (this.num === '') {
					...
					.then(() => {
						var that = this
						this.axios.get(`${this.url}/system/smsmanage/delete`)
						.then(function (response) {
							that.total = that.tableData.length
							that.total = that.tableData.length
						})
...
<style>
	...
	.message-management .card-content table{
		margin-top: 10px;
		width: 100%;
	}
	.message-management .card-content table tr th{
		width: 100px;
		height: 32px;
		padding: 1px 0;
		text-align: right;
	}
	.message-management .card-content table tr td{
		text-align: left;
		height: 34px;
		padding: 1px 0 1px 5px;
		line-height: 32px;
	}
	.message-management .export{
		height: 31px;
		padding-left: 5px;
		border-bottom: 1px solid #ddd;
		background-color: #f4f4f4;
	}
	.message-management .icon{
		padding-left: 2px;
		float: left;
		width: 78px;
		height: 26px;
	...
	.message-management #isSelect{
		width: 15px;
		height: 18px;
	}
	.message-management table a{
